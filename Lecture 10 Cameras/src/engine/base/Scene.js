...
class Scene extends NameableParent {
    ...
    draw(ctx, width, height) {
        ...
        let cameraGameObjects = this.children.filter(i=>i.hasComponent("CameraComponent"));
        if (cameraGameObjects.length == 0){
            console.log("No Camera Present in Scene");
        } else {
            let camera = cameraGameObjects[0];
